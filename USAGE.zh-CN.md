# Dig UI Skill: 设计系统维护与使用指南

[English](./USAGE.md) · [中文首页](./README.zh-CN.md) · [中文安装指南](./INSTALL.zh-CN.md)
...
- **Catalog (`references/catalogs/`)** 是以成熟产品 UI 语言为入口的基础 catalog。每个页面或组件组只选择一个基础 catalog。
...
- **Style (`references/catalogs/styles/` + `~/.config/dig-ui-skill/styles/`)** 是以完整视觉语法为入口的 catalog 线。内置 style 属于仓库资产，用户从 Style Lab 导入的 custom style 属于仓库外个人资产。
...
你可以直接告诉 Agent 希望长期保留的 UI 判断。它会按 `references/local-rules-builder.md` 将偏好写入用户资产，不需要 fork 官方 catalog，也不需要额外配置 AI API key：
...
更多安装细节见 [INSTALL.zh-CN.md](./INSTALL.zh-CN.md)。
...
```bash
# 导入导出的 JSON、Markdown 或 ZIP，并同步到 Codex
...
| Local Extensions | `references/local/` | `extends` / overrides / synced user palettes and styles 文档化维护 |
...
1. **更新说明文档**
   如果这是正式新增到布局库的 recipe，同步更新 `references/layouts/README.md` 的 layout 库表格。
...
- **智能分类导航标签 (Tabs)**：以毛玻璃卡片和极高保真的排版设计，将 catalog 预览有序归类为 AI 平台、开发工具、DevOps、金融科技、色彩目录、style catalog 等分类。
