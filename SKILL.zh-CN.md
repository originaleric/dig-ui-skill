...
- `apple`：高端产品发布、系统原生感
- `paletteXX`：以整体网站配色、mood 或颜色组合为入口的 color palette catalog
...
用户指定品牌、产品或成熟视觉语言时使用 brand catalog；用户指定 mood、整体 palette 或颜色组合时使用 palette catalog。同一页面或同一组组件不要混用多个 base catalog。
